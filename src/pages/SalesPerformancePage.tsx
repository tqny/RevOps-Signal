    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-4">
        <MetricTile
          ...
          detail={`${formatCompactCurrency(performance.totalPipelineAmount)} open pipeline across ${formatCount(performance.totalOpenDealCount)} active deals in the current scope.`}
          tone="neutral"
        />
        <MetricTile
          label="Closed-won attainment"
          value={formatPercentage(performance.attainmentRate)}
          detail={`${formatCompactCurrency(performance.totalClosedWonAmount)} booked, ${formatSignedCompactCurrency(-performance.attainmentGapAmount)} versus target.`}
          tone={performance.attainmentRate >= 1 ? 'success' : 'warning'}
        />
        <MetricTile
          label="Forecast coverage"
          value={formatPercentage(performance.forecastAttainmentRate)}
          detail={`${formatCompactCurrency(performance.totalWeightedForecastAmount)} weighted forecast, ${formatSignedCompactCurrency(-performance.forecastGapAmount)} versus target.`}
          tone={
        ...
        <MetricTile
          label="On-pace reps"
          value={onPaceValue}
          detail={`${formatCount(performance.repsNearTargetCount)} more reps are in the 80-99% working band.`}
          tone={performance.repsAtOrAboveTargetCount > 0 ? 'success' : 'warning'}
        />
      </section>

      {!performance.hasResults ? (
        <SurfaceCard title="Sales performance">
          <EmptyState
        ...
        <>
          ...
          <section className="grid gap-4 2xl:grid-cols-[minmax(0,1.02fr)_minmax(0,0.98fr)]">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-text-secondary">
                        Each team bar compares weighted forecast to visible
                        quota.
                      </p>
                    </div>
                    <StatusBadge variant="neutral">
            ...
            <SurfaceCard
              ...
              footer={`${formatCount(performance.repsOffPaceCount)} reps are still below the 80% pace band in the current scope.`}
            >
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          <p className="text-xs text-text-muted">
                            {row.regionName} • {formatCount(row.openDealCount)}{' '}
                            open deals
                          </p>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          ...
                          <p className="text-xs text-text-muted">
                            {formatPercentage(row.forecastAttainmentRate)} of
                            quota
                          </p>
                        </td>
                        <td className="px-4 py-3 text-right text-text-primary">
